...
3. **Pre-flight 资料搜集**（生成卡片后、派发 executor 前）：遍历本工作流 leaf skill 的 `## Inputs` 节，汇总**资料类**输入（用户文件/路径 + 关键 choice）。小需求工作流的三个 skill（澄清 / 旅程 / TR1）输入多为上游 artifact 或路由阶段已给的需求——**若无用户文件/路径类输入，直接跳过、不提问**，仅写 `$RUN_DIR/pre-flight/collected-inputs.json: {"skipped": "no user-material inputs"}`；若有，则一次性向用户呈现清单并写入该 artifact，再注入对应 task card 的 `## Input Artifacts`（缺料项标注降级）。完整流程见 `large-requirement-workflow` 步骤 3。
4. **调用 `cospec-dag-executor`** 派发 skill-invoker SubAgents。
5. **等待完成**并汇总交付物。
...
**生成规则**（对 DAG 表每一行）：`Skill`=节点名；`Source`=`小需求工作流 — <step> <职责>`；`Depends on`=该行 `depends_on`（无则 `(none)`）；`Input Artifacts`=`depends_on` 各节点 manifest 路径（+ Pre-flight 注入的用户材料，见步骤 3）；`Task Spec`=调用 `<skill>` 遵循其 SKILL.md；`Required Output Artifacts`=`.cospec/runs/<RUN_DIR>/<skill>/manifest.json` + `results.md`。
...
- 禁止修改业务 DAG 的节点集合与依赖拓扑——`dag.json` 中的 `tasks`（3 个节点）及其 `depends_on` 必须与「工作流 DAG」表一致；执行流编排步骤（生成产物、评估、pre-flight、派发、汇总）可按需增改。
