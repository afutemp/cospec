...
test('all six user skills have frontmatter and core is not triggerable', () => {
  ...
  assert.equal(fs.existsSync(path.join(root, 'skills', 'product-kb-core')), false);
});

test('update and optimize have separate confirmation gates', () => {
  assert.match(read('skills/product-kb-update/SKILL.md'), /等待用户明确确认/);
  assert.match(read('skills/product-kb-optimize/SKILL.md'), /等待用户确认/);
  assert.match(read('skills/product-kb/SKILL.md'), /分别确认/);
});

test('dispatcher covers lifecycle routing states', () => {
  const content = read('skills/product-kb/SKILL.md');
