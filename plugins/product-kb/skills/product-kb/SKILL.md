...
| 建索引、生成目录/INDEX、让 agent 更精准找文档 | 任意含 `.md` 的知识库目录（含外部源文档） | `product-kb-index` |
...
- 索引随文档：Init 和 Update 在校验通过后自动路由 `product-kb-index` 重建 `INDEX.md`，无需单独触发；对外部非受管源文档才显式调用 `product-kb-index`。
