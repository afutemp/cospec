...
1. 按上表顺序，依次调用每个规划 leaf skill：`Skill("<skill-name>")`，并记录它实际生成的产物路径。不主动询问跳过；用户主动要求跳过某节点时允许跳过，跳过 step3 TR1 前说明将失去终点产物。
...
- 3 个节点默认全部执行、不主动询问跳过；但用户主动要求跳过某节点时允许跳过。跳过 step3 TR1 会让本工作流失去终点产物，须先向用户说明该后果、由用户确认后再执行跳过
- 禁止修改节点顺序
