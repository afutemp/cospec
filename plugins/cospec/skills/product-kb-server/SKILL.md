...
不要默认使用任何固定知识库名称。先列出服务器上所有可用的知识库，让用户选择（或用户提供名称/ID）后再下载。

**1. 列出所有知识库：**

```bash
./skills/product-kb-server/scripts/product-kb-server.cjs list
# 输出表格：ID | NAME | STATUS | VERSION
```
...
如果用户没有指定知识库名称，调用 `list` 并将结果展示给用户，等待用户选择后再执行 `download`。
