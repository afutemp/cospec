...
如果用户想删除本地已下载的知识库（例如切换 KB 或清理旧版本），使用 `delete` 命令：

```bash
./skills/product-kb-server/scripts/product-kb-server.cjs delete --kb <kb-name-or-id>
```

- 默认删除 `~/.cospec/kb/<kb-name>/` 目录。
- 如果 `cospec.config.json` 的 `kb.localPath` 正好指向该目录，会自动清空 `kb.localPath`。
- 为了安全，删除非默认 `--output` 目录时**不会**自动清理 `kb.localPath`。
...
./skills/product-kb-server/scripts/product-kb-server.cjs delete --kb <kb-name-or-id>
# 输出示例：
# [delete] vdi: removed /home/user/.cospec/kb/vdi
# [config] cleared kb.localPath in /path/to/cospec.config.json
```
