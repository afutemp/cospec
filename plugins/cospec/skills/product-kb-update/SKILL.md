...
2. 使用 `scripts/product-kb-core/scripts/collect.js` 采集到 `.source/pending-update/`，包括重新下载和解析范围内需求附件。附件正文、文件指纹或解析状态变化必须进入快照差异。
...
9. 校验通过后，路由 `product-kb-index`（`root` = 本知识库根）重建 `INDEX.md`，使索引随本次变更保持与正文一致。索引重建/校验最多修补两轮；仍失败不回滚正文，但**最终状态门禁**须区分：索引 `--verify` 通过 → 报告"更新完成，索引已同步"；索引缺失或未通过校验 → 报告"更新完成，但索引未同步，可重跑 `product-kb-index`"。不得在索引未同步时笼统声称更新全部完成。
