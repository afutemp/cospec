...
本工作流对应 `docs/workflow-skill-steps.md` 中的「大需求工作流」。**所有节点均为必选**——不对研究/竞品节点做按需裁剪；运行时若缺关键输入，由对应 skill 通过 `NEEDS_CONTEXT` 向用户索取。
...
> step2 的 5 个研究 skill 均为状态机型；运行时若分析范围未定或缺少关键输入，会返回 `NEEDS_CONTEXT` 向用户索取（executor 问询队列错峰处理）。这是必选节点的正常行为——索取材料，而非裁剪节点。
...
| `Input Artifacts` | `depends_on` 各节点的 `.cospec/runs/<RUN_DIR>/<dep>/manifest.json` |
...
- 禁止裁剪任何必选节点——5 个研究 skill 均必选；缺少材料时由 skill 通过 `NEEDS_CONTEXT` 向用户索取，而非移除节点。
- 禁止在 `cospec-dag-executor` 报告 `BLOCKED` 且未升级时继续推进。
- 禁止修改业务 DAG 的节点集合与依赖拓扑——`dag.json` 中的 `tasks`（12 个必选节点）及其 `depends_on` 必须与「工作流 DAG」表一致；执行流编排步骤（生成产物、评估、派发、汇总）可按需增改。
