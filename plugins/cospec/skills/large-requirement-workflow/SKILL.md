...
**所有节点默认执行**。跳过规则按 step 类型区分：多 skill 的 step（step2 的 5 个研究、step5 的 4 个 TR2）进入前主动询问用户跳过哪些；单个 skill 的 step（step1/3/4）**不主动询问、直接执行**，但**用户主动要求跳过时允许跳过**。跳过 step4 的 TR1 会让 step5 的 TR2 失去必需来源（TR1 AI 上下文版），须先向用户说明该后果、由用户确认后再执行跳过。
...
1. **进入多 skill 的 step 前**（step2、step5）：列出当前 step 内的全部 skill，询问用户 **"以下哪些需要跳过？"**，用户选择后只执行剩余的 skill。单个 skill 的 step（step1/3/4）不主动询问、直接执行；但若用户在进入或确认环节**主动要求跳过**，允许跳过该步并继续下一步。跳过 step4 TR1 时，必须先说明"step5 的 TR2 将失去必需来源（TR1 AI 上下文版）"，由用户确认后再跳过。
...
- 单个 skill 的 step（step1/3/4）不主动询问跳过，但禁止在用户主动要求跳过时拒绝——必须允许跳过；跳过 step4 TR1 前必须先说明对 step5 TR2 的影响
