...
本工作流对应 `docs/workflow-skill-steps.md` 中的「大需求工作流」。**所有节点均为必选**——不对研究/竞品节点做按需裁剪；用户需提供的资料由步骤 3 的 pre-flight 一次性收集并注入卡片；运行时若仍缺关键输入，由对应 skill 通过 `NEEDS_CONTEXT` 兜底索取。
...
> step2 的 5 个研究 skill 均为状态机型；其资料类输入由步骤 3 pre-flight 预先收集。若 pre-flight 未覆盖或分析范围未定，运行时仍会返回 `NEEDS_CONTEXT` 向用户索取（executor 问询队列错峰处理）。这是必选节点的正常行为——索取材料，而非裁剪节点。
...
3. **Pre-flight 资料搜集**（生成卡片后、派发 executor 前）——避免各研究 skill 运行时才逐个 `NEEDS_CONTEXT` 索要材料：
   1. 遍历 step2 的研究类 leaf skill，读取各自 SKILL.md 的 `## Inputs` 节。
   2. 只汇总**资料类**输入：用户文件/路径（纪要、访谈转写、反馈数据文件）+ 关键 choice（分析模式 single/multi、单/多客户、需求类型）。**排除**上游 artifact 类输入（user-journey 依赖的研究产出等，由 manifest 流转）。
   3. 去重后一次性向用户呈现资料清单，每项说明「为什么要」+「影响什么」（复用 `product-planning-requirement-clarification` 的 bounded 提问风格）。
   4. 将结果写入 `$RUN_DIR/pre-flight/collected-inputs.json`：按 `<skill-id>` → `<input-name>` 组织，每个条目为 `{ "provided": true, "value": "<path 或内容指针>" }` 或 `{ "provided": false, "note": "用户未提供 <X>" }`。
   5. 把该 artifact 路径与各 skill 条目注入对应 task card 的 `## Input Artifacts`：已提供 → 写入路径/内容指针；未提供 → 写入「用户未提供 <X> → 该 skill 降级产出（标注证据不足/待确认）」。能否降级、如何降级由 leaf skill 自身契约决定。
4. **调用 `cospec-dag-executor`** 派发 skill-invoker SubAgents（按 ready-set 并行；同一 step 的 skill 落在同一 ready set 一次性并发派发；`max_parallel_tasks` ≥ 5）。
5. **等待完成**并汇总交付物。
...
| `Input Artifacts` | `depends_on` 各节点的 `.cospec/runs/<RUN_DIR>/<dep>/manifest.json` + Pre-flight 注入的用户材料（见步骤 3；缺料项标注降级） |
...
- <pre-flight: user-provided materials from .cospec/runs/<RUN_DIR>/pre-flight/collected-inputs.json; missing items → degraded output>
...
- 禁止裁剪任何必选节点——5 个研究 skill 均必选；缺少材料时由 pre-flight 预收集/标注降级，或由 skill 通过 `NEEDS_CONTEXT` 兜底索取，而非移除节点。
- 禁止在 `cospec-dag-executor` 报告 `BLOCKED` 且未升级时继续推进。
- 禁止修改业务 DAG 的节点集合与依赖拓扑——`dag.json` 中的 `tasks`（12 个必选节点）及其 `depends_on` 必须与「工作流 DAG」表一致；执行流编排步骤（生成产物、评估、pre-flight、派发、汇总）可按需增改。
