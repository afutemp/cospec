...
`sync-to-ipd` 也是独立 Skill，不接入 `brainstorming`。它负责 cospec 产物发现、稳定 ID／层级校验、目标选择、预览和最终确认；IPD 查询和写入通过本 Skill 的 `scripts/ipd-provider.cjs` 确定性调用同一 cospec 插件内的 `qianliu-ipd` 脚本，不依赖嵌套 Skill 或 MCP 工具。集成必须保留以下边界：预览零 IPD 写入、同名不自动绑定、计划哈希漂移失效、写失败不自动重试、TR1 评审版优先交付物且 AI 上下文版进入根 Epic 附件。
