...
`sync-to-ipd` 也是独立 Skill，不接入 `brainstorming`。它只负责 cospec 产物发现、稳定 ID／层级校验、目标选择、预览和最终确认；IPD 查询和写入委托 `qianliu-ipd`。集成必须保留以下边界：预览零 IPD 写入、同名不自动绑定、计划哈希漂移失效、写失败不自动重试、TR1 评审版优先交付物且 AI 上下文版进入根 Epic 附件。
