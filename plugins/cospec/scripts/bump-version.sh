...
CONFIG="$PLUGIN_ROOT/.version-bump.json"
...
cmd_check() {
  ...
  while IFS=$'\t' read -r path field; do
    local fullpath="$PLUGIN_ROOT/$path"
    if [[ ! -f "$fullpath" ]]; then
      printf "  %-45s  MISSING\n" "$path ($field)"
      has_drift=1
      continue
...
cmd_audit() {
  ...
  while IFS= read -r match; do
    ...
    local rel_path="${match_file#$PLUGIN_ROOT/}"
  ...
  done < <(grep -rn "${exclude_args[@]}" -F "$current_version" "$PLUGIN_ROOT" 2>/dev/null || true)
